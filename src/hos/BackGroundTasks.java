...
public class BackGroundTasks implements Runnable{
	...
	public ArrayList<byte[]> getTimeStampList(ArrayList<String> fileNames){
		...
		for (int i=0; i<nFiles; i++){
			...
			String rowKey = ","  + fileNames.get(i).substring(0, indexOfUnder);
			System.out.println("row : " + rowKey);
			temp = HbaseUtil.getResultsContaining(DBStrings.Table_objectsTableString, rowKey);
			for(Result r : temp)
	...
	public String genHarFileName(){
		...
		try {
			if (fs.exists(new Path("/hos/harFiles/" + newHarName))){
...
//		TODO change this to greater than 
		if (smallFilesSize>ObjectProtocol.ARCHIVE_SIZE_THRESHOLD){
			...
			String harDirPath = "/hos/harFiles" ;
			...
			for (int i=0; i<files.size(); i++){	
				...
				try{
					...
					Result tempResult = HbaseUtil.getResultsContaining(DBStrings.Table_objectsTableString, fileName).get(0);
					String actualKey = Bytes.toString(tempResult.getRow());
					String entryName = removeUnderScore(fileName);
					String rowKey = (actualKey.substring(0, actualKey.indexOf(','))) + "," + entryName;
					System.out.println("rowKey : " + rowKey);
					Put p = new Put(Bytes.toBytes(rowKey));
					p.addColumn(Bytes.toBytes(DBStrings.DEFAULT_COLUMN_FAMILY), Bytes.toBytes(DBStrings.Col_url), 
							Bytes.toBytes("har:///hos/harFiles/" + harFileName + "/" + fileName));
					...
					if (!flag){
						...
						String corrHDFS = "/hos/smallFiles/"+ fileName;
						...
						String harHDFS = "/hos/harFiles" + harFileName;
			...
			for (int i=0; i<files.size(); i++){
				String fileName = files.get(i);
				String path = "/hos/smallFiles/" + fileName;
				try {
...
//	TODO pick har file
	...
	public void unarchive(){
		...
		hm.unArchiveAll(chosenHarPath, "/hos/smallFiles");
		...
		for (int i=0; i<files.size(); i++){	
			try{
				...
				Put p = new Put(Bytes.toBytes(rowKey));
				p.addColumn(Bytes.toBytes(DBStrings.DEFAULT_COLUMN_FAMILY), Bytes.toBytes(DBStrings.Col_url), 
						Bytes.toBytes("/hos/smallFiles/" + fileName));
				p.addColumn(Bytes.toBytes(DBStrings.DEFAULT_COLUMN_FAMILY), Bytes.toBytes(DBStrings.Col_timeStamp), 
						Bytes.toBytes(String.valueOf(System.currentTimeMillis() ) ) );
				boolean flag = table.checkAndPut(Bytes.toBytes(rowKey), Bytes.toBytes(DBStrings.DEFAULT_COLUMN_FAMILY), Bytes.toBytes(DBStrings.Col_timeStamp),
						timeStampList.get(i), p);
